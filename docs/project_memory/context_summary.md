...
**Last updated:** 2026-08-31 — **TIME-315 (Jira TIME-2349) the recording waveform now reacts to your voice.** Straight after TIME-314 landed and voice capture was confirmed working on the phone, the waveform was live but read as static. Two causes, both only visible in the arithmetic: the idle shimmer was CLAMPED AWAY (`0.16 * jitter` = 2.6–7.4pt against a 6pt `minHeight` floor, so silence rendered as motionless dots — the same live-vs-dead ambiguity that hid TIME-314 for weeks), and speech was drawn with per-bar `CGFloat.random` re-rolled every 0.11s, which reads as flicker rather than response. Plus linear `rms * 18` left ordinary speech (~0.02 RMS) at only 0.36. Now: `TimelineView(.animation)` driving one continuous travelling wave at display rate, centre-weighted bars, a peak-hold envelope so a syllable survives the ~14/s publish rate, curved loudness (speech 0.36 → 0.62), and a hero glow tracking volume. Awaiting the user's visual sign-off — the idle floor, wave speed and circle pulse are independently tunable.
...
- **TIME-315 → Jira TIME-2349** (waveform reacts to your voice; 2026-08-31). Depends on TIME-314.
...
## Current Active Task
**TIME-315 (Jira TIME-2349)** — waveform reactivity. Implemented, tested, built/installed/launched on
the phone, awaiting the user's visual judgement. Branch `feature/TIME-315-waveform-reacts-to-voice`.

**TIME-314 (Jira TIME-2348) is DONE** — merged as PR #354 and **confirmed working on the user's
iPhone**. The stale-engine diagnosis was correct: rebuilding `AVAudioEngine` per session fixed it.

TIME-308..313 still need **rebuilding onto the physical device** and confirming in use.

Device tooling that works here (learned 2026-08-31, `xcodebuild -showdestinations` for the UDID):
- `xcodebuild ... -destination 'id=00008110-0014703A0C7A401E' -allowProvisioningUpdates build`
  (the hardware UDID, NOT the `devicectl list devices` coredevice identifier — that one is rejected)
- `xcrun devicectl device install app --device 00008110-0014703A0C7A401E <path>/TimeSense.app`
- `xcrun devicectl device process launch --device 00008110-0014703A0C7A401E com.aetheranalytics.timesense`
- **The phone must be UNLOCKED**: a locked phone aborts install mid-transfer and refuses launch.
- `log stream --device*` is gone from this macOS and devicectl has no console subcommand, so reading
  os_log from the device means Console.app.
