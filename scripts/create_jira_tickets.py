...
TICKETS = [
    ...
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Dependencies"), p("TIME-222 (calendar->task), push infra, maps skill."), divider(), h2("Next Ticket"), p("TIME-252."),
        ),
    },
    {
        "summary": "TIME-252: Behavioral-data models + consent-gated ingest (workouts + hourly activity)",
        "labels": ["backend", "health", "insights", "feature"],
        "description": doc(
            h2("Goal"), p("Store the time-series Apple Health data needed for behavioral patterns on the Insights screen: workout sessions (runs + gym) and hourly step buckets (for sit-vs-move). Foundation for TIME-253."),
            divider(), h2("Scope"), bullet_list([
                "New WorkoutSession model (user_id, external_id unique-per-user, workout_type running/walking/cycling/strength/hiit/other, started_at, ended_at, duration_minutes, distance_meters?, active_energy_kcal?, source) + repo + migration",
                "New HourlyActivity model (user_id, hour_start unique-per-user, steps, source) + repo + migration",
                "POST /api/v1/activity/workouts and POST /api/v1/activity/hourly — batch upsert, gated on health_data consent (copy the sleep 403 pattern)",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No aggregation/patterns yet (TIME-253); no change to the existing daily /activity endpoint"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend/app/models/workout_session.py, hourly_activity.py (new)", "repositories + migrations", "backend/app/api/v1/activity.py + schema", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Both endpoints upsert+dedup and 403 without health_data consent; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_activity.py -q && pytest -q"),
            divider(), h2("Dependencies"), p("Health pipeline (DailyActivity)."), divider(), h2("Next Ticket"), p("TIME-253."),
        ),
    },
    {
        "summary": "TIME-253: BehavioralPatternsService + GET /insights/patterns",
        "labels": ["backend", "insights", "feature"],
        "description": doc(
            h2("Goal"), p("Compute plain-language behavioral patterns over ~28 days for the Insights screen: running (miles/week, duration, typical times), gym (frequency/days/times), sit-vs-move (ratio, typical sit windows + streak length), and driving (commute time/day & /week from confirmed commutes)."),
            divider(), h2("Scope"), bullet_list([
                "New BehavioralPatternsService.for_user(user_id, tz) (mirror LearnedPreferencesService) reading WorkoutSession/HourlyActivity/CommuteRepository/DailyActivity",
                "Add CommuteRepository.sum_confirmed_minutes_in_range alongside count_confirmed_in_range",
                "GET /api/v1/insights/patterns (PremiumUser) -> {patterns:[{category,icon,title,detail}], based_on_days}; only emit items past a small data threshold",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No new ingest (TIME-252); driving is commute-only (labelled honestly)"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend/app/services/behavioral_patterns_service.py (new)", "backend/app/api/v1/insights.py + schema", "commute_repository.py", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Seeded workouts/hourly/commute rows produce correct plain-language patterns with thresholds; premium-gated; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_behavioral_patterns.py -q && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-252."), divider(), h2("Next Ticket"), p("TIME-254."),
        ),
    },
    {
        "summary": "TIME-254: iOS HealthKit workout + hourly-step reads & sync",
        "labels": ["ios", "health", "feature"],
        "description": doc(
            h2("Goal"), p("Collect the data TIME-252 stores: read Apple Health workouts (runs + gym) with distance/duration/time, and hourly step buckets (currently computed then discarded), and sync them to the new endpoints."),
            divider(), h2("Scope"), bullet_list([
                "HealthService: add HKWorkoutType + distanceWalkingRunning to readTypes",
                "Enumerate recent HKWorkouts (~28d) -> POST /activity/workouts (map HKWorkoutActivityType -> workout_type)",
                "HKStatisticsCollectionQuery over ~14d in 1-hour intervals -> POST /activity/hourly",
                "Call from connectAndSync() + app-launch sync",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No background HKObserverQuery delivery"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios/TimeSense/Core/Health/HealthService.swift"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["iOS builds; workouts + hourly steps POST to the new endpoints (HealthKit reads need a real device to fully verify)"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16'"),
            divider(), h2("Dependencies"), p("TIME-252 (endpoints)."), divider(), h2("Next Ticket"), p("TIME-255."),
        ),
    },
    {
        "summary": "TIME-255: Insights 'Your patterns' section (iOS + web)",
        "labels": ["ios", "web", "insights", "feature"],
        "description": doc(
            h2("Goal"), p("Surface the behavioral patterns on the Insights screen (iOS Insights tab + web insights page)."),
            divider(), h2("Scope"), bullet_list([
                "iOS InsightsView/InsightsViewModel: a 'What we've learned about you' section fetching GET /insights/patterns (best-effort), category-grouped rows (icon+title+detail)",
                "web insights page: a patterns card (mirror the existing learned block)",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No Settings Learned Patterns change (lives on Insights)"]),
    ...
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Verification"), code_block("cd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-259 (APNs secrets)."), divider(), h2("Next Ticket"), p("TIME-265."),
        ),
    },
    {
        "summary": "TIME-265: Why-sheet Calendar signal names the next commitment (not 'workday ends')",
        "labels": ["backend", "recommendations", "bug"],
        "description": doc(
            h2("Goal"), p("The Calendar signal defaults to 'N minutes free before your workday ends' because _free_and_next only counts tasks with a scheduled_start today (UTC). Name the user's actual next commitment — a task with a due_at or scheduled_start (manual/email/Notion), or a calendar event — in their local day."),
            divider(), h2("Scope"), bullet_list([
                "TaskRepository.upcoming_commitments(user_id, now, local_day_end): not done/cancelled, scheduled_start in range OR due_at in range",
                "_free_and_next: build next-commitment from those (scheduled_start else due_at) + timed SyncedCalendarEvents in local day; earliest after now; fall back to workday-end only when none",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No ranking change"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend/app/services/recommendation_explainer.py", "backend/app/repositories/task_repository.py", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["A due-only email/Notion/manual task or a calendar event is named as the next commitment; workday-end only when truly none; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_explanation_reasoning.py -q"),
            divider(), h2("Dependencies"), p("TIME-243."), divider(), h2("Next Ticket"), p("TIME-266."),
        ),
    },
    {
        "summary": "TIME-266: Why-sheet Energy varies with activity (was constant 'moderate')",
        "labels": ["backend", "recommendations", "bug"],
        "description": doc(
            h2("Goal"), p("The activity fallback in _health returns 'moderate' always. Derive energy from DailyActivity (steps/exercise/inactive) so it varies."),
            divider(), h2("Scope"), bullet_list(["_health activity path + now._context_cards fallback: high when exercise>=30 or steps>=~8k; low when very sedentary; else moderate; detail names the driver"]),
            divider(), h2("Non-Goals"), bullet_list(["No new HealthKit reads"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend/app/services/recommendation_explainer.py", "backend/app/api/v1/now.py", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Energy is high/moderate/low based on activity; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_explanation_reasoning.py -q"),
            divider(), h2("Dependencies"), p("TIME-246."), divider(), h2("Next Ticket"), p("TIME-267."),
        ),
    },
    {
        "summary": "TIME-267: Remove the non-functional chevron in 'Alternatives considered'",
        "labels": ["ios", "recommendations", "bug"],
        "description": doc(
            h2("Goal"), p("The alternatives rows in the Why sheet show a chevron.right but do nothing when tapped. Remove it (informational rows)."),
            divider(), h2("Scope"), bullet_list(["NowView AlternativesCard: drop the chevron"]),
            divider(), h2("Non-Goals"), bullet_list(["No change to 'Other good options' (that list is tappable)"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios/TimeSense/Features/Now/NowView.swift"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["No misleading arrow on alternatives rows; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-268."),
        ),
    },
    {
        "summary": "TIME-268: Breathing room between content and the bottom tab bar",
        "labels": ["ios", "polish"],
        "description": doc(
            h2("Goal"), p("Scrolled content sits flush against the custom tab bar. Add bottom padding so the last item clears the bar."),
            divider(), h2("Scope"), bullet_list(["Increase bottom padding on the main scroll views (Now/Today/Insights/Capture/Settings)"]),
            divider(), h2("Non-Goals"), bullet_list(["No tab-bar redesign"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios main feature views"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Clear gap above the tab bar at the bottom of scroll; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
            divider(), h2("Dependencies"), p("TIME-239 (tab bar)."), divider(), h2("Next Ticket"), p("TIME-269."),
        ),
    },
    {
        "summary": "TIME-269: Nav title gains a subtle background on scroll",
        "labels": ["ios", "polish"],
        "description": doc(
            h2("Goal"), p("The nav bar is fully transparent so content scrolls under the fixed title. Give it an adaptive background once scrolled (standard iOS)."),
            divider(), h2("Scope"), bullet_list(["TimeSenseApp.init: standardAppearance = default/material background; keep scrollEdgeAppearance transparent"]),
            divider(), h2("Non-Goals"), bullet_list(["No large-title restyle"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios/TimeSense/App/TimeSenseApp.swift"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["At top: clear over the cosmic bg; on scroll: subtle bar, content no longer overlaps the title; both schemes; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
            divider(), h2("Dependencies"), p("TIME-260 (chrome)."), divider(), h2("Next Ticket"), p("TIME-270."),
        ),
    },
    {
        "summary": "TIME-270: Reduce the sparkles icon to the AI-recommendation marker only",
        "labels": ["ios", "design", "polish"],
        "description": doc(
            h2("Goal"), p("The sparkles SF Symbol is used ~16x, which reads as generic-AI. Keep it only as the consistent AI-recommendation marker; replace incidental uses with domain-appropriate icons."),
            divider(), h2("Scope"), bullet_list(["Audit sparkles usages; keep on the Now AI hero / 'Why This Recommendation?'; replace elsewhere (tab icon, Capture, Today, Onboarding, Settings, TimelineCard, Shortcuts)"]),
            divider(), h2("Non-Goals"), bullet_list(["No behavior change"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios various feature views + MainTabView"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Sparkles appears only where it marks the AI recommendation; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-271."),
        ),
    },
    {
        "summary": "TIME-271: Disagree — reliable swap + reason capture + reason-based learning (backend)",
        "labels": ["backend", "recommendations", "feature"],
        "description": doc(
            h2("Goal"), p("Disagree should reliably remove the pick (show the next-best), capture WHY, and learn from it. Today it's a -30 penalty a strong pick survives, with no reason capture."),
            divider(), h2("Scope"), bullet_list([
                "Exclude recently-disagreed task ids (3h window) from the BEST-pick selection on /now (still allowed as alternatives)",
                "Add reason (nullable) to RecommendationFeedback (+ migration) + FeedbackRequest schema",
                "Map reasons -> scoring signals (wrong_time -> AVOIDED_AT_THIS_TIME; not_priority -> deprioritize action type; not_relevant -> stronger/longer suppression; too_big -> bias shorter)",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No change to agree/done/snooze"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend recommendation feedback model/repo/schema + migration, penalties.py, build_summary.py, now.py best-pick selection", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["A disagreed task isn't the top pick within the window; reason persists; reason affects scoring; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_now.py tests/test_feedback.py -q"),
            divider(), h2("Dependencies"), p("TIME-185..188 (agree/disagree)."), divider(), h2("Next Ticket"), p("TIME-272."),
        ),
    },
    {
        "summary": "TIME-272: Disagree — reliable next pick + 'why?' prompt (iOS)",
        "labels": ["ios", "recommendations", "feature"],
        "description": doc(
            h2("Goal"), p("After tapping Disagree, show the next-best (backend guarantees it) and ask a quick reason that feeds learning."),
            divider(), h2("Scope"), bullet_list(["NowViewModel/NowView: after disagree, a reason prompt (Wrong time / Not a priority / Not relevant / Too big / Skip); POST feedback with reason"]),
            divider(), h2("Non-Goals"), bullet_list(["No change to Agree"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios NowView.swift, NowViewModel.swift"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Disagree swaps to a different pick and asks why; reason is sent; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
            divider(), h2("Dependencies"), p("TIME-271."), divider(), h2("Next Ticket"), p("TIME-273."),
        ),
    },
    {
        "summary": "TIME-273: Insights chart series endpoints (activity/workouts/hourly)",
        "labels": ["backend", "insights", "charts"],
        "description": doc(
            h2("Goal"), p("Expose read-only series for the Insights charts. Weekly history already exists (GET /insights/history); add activity/workout/hourly series."),
            divider(), h2("Scope"), bullet_list([
                "DailyActivityRepository.list_in_range (new) + GET /insights/activity (daily steps/exercise)",
                "GET /insights/workouts (weekly running miles + counts from WorkoutSession.list_in_range)",
                "GET /insights/hourly (24-bucket steps for sit-vs-move)",
                "All premium-gated",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No new ingestion; weekly-history endpoint unchanged"]),
            divider(), h2("Files Likely Changed"), bullet_list(["backend/app/api/v1/insights.py + schemas", "daily_activity_repository.py", "tests"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Each endpoint returns a chartable series, premium-gated; suite green"]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_behavioral_patterns.py -q && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-252/253 (activity/workouts)."), divider(), h2("Next Ticket"), p("TIME-274."),
        ),
    },
    {
        "summary": "TIME-274: Insights charts UI (Swift Charts)",
        "labels": ["ios", "insights", "charts"],
        "description": doc(
            h2("Goal"), p("Add beautiful charts to the Insights screen from the data we return."),
            divider(), h2("Scope"), bullet_list([
                "Weekly (from /insights/history): completion-rate line, tasks completed vs total bars, acceptance % + confidence lines",
                "Activity/workouts (TIME-273): running miles/week bars, daily steps line, sit-vs-move ring/bars",
                "Theme-aware (light+dark); graceful when a series is empty",
            ]),
            divider(), h2("Non-Goals"), bullet_list(["No new metrics beyond what endpoints return"]),
            divider(), h2("Files Likely Changed"), bullet_list(["ios InsightsView.swift, InsightsViewModel.swift"]),
            divider(), h2("Acceptance Criteria"), bullet_list(["Charts render for weekly + activity/workout data in both schemes; iOS builds"]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense"),
    ...
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_now.py -q"),
            divider(), h2("Dependencies"), p("TIME-279."), divider(), h2("Next Ticket"), p("TIME-282."),
        ),
    },
    # ── RECOMMENDATION QUALITY BATCH (device feedback, 2026-08-28) ───────────
    {
        "summary": "TIME-282: Durable entitlement override for owner/comped accounts",
        "labels": ["backend", "subscriptions", "ios", "bug"],
        "description": doc(
            h2("Goal"), p("The owner's own account is gated behind Premium. Root cause: no client ever calls POST /subscriptions/trial, so no Subscription row exists and is_premium is decided entirely by user.created_at + intro_trial_days (14). Accounts older than that fall off a cliff. The only escape hatch, PREMIUM_TEST_EMAILS, is set in the local .env but is absent from render.yaml, so it is empty in production. Separately the iOS Subscription settings screen reads GET /subscriptions/me, which returns null for anyone without a row, so it displays 'Basic (Free)' even for a genuinely entitled trial user. Give comped/staff accounts a durable, non-string-matched entitlement and make the settings screen tell the truth."),
            divider(), h2("Scope"), bullet_list([
                "New nullable User.entitlement_override column (comped | staff), default null, + Alembic migration with server_default now() on timestamps",
                "SubscriptionService.is_premium checks entitlement_override FIRST, before the subscription row, intro trial, and email allowlist",
                "Add PREMIUM_TEST_EMAILS to render.yaml's timesense-secrets env var group (sync: false) as the immediate production unblock",
                "iOS SettingsScreens Subscription view reads GET /subscriptions/me/entitlement (handles the rowless case) instead of /subscriptions/me; Android mirror",
                "Admin-only PATCH /api/v1/admin/users/{id}/entitlement to set/clear the override, guarded by the existing require_admin dependency",
                "Extend scripts/check_premium.py to report the override",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No StoreKit / Google Play purchase flow — the 'Upgrade to Premium' buttons remain stubs",
                "No graceful Basic Mode degradation of the 403-gated endpoints (separate follow-up)",
                "No change to intro_trial_days or the trial model itself",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/models/user.py, app/services/subscription_service.py, app/api/v1/admin.py, app/schemas/user.py",
                "backend migrations/versions/<new>.py",
                "render.yaml, scripts/check_premium.py",
                "ios TimeSense/Features/Settings/SettingsScreens.swift; android AppViewModel.kt",
                "backend tests/test_subscriptions.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A user with entitlement_override='comped' is_premium regardless of account age or subscription row",
                "scripts/check_premium.py reports the override and the final is_premium verdict",
                "iOS Subscription settings shows 'Premium' for an entitled trial user with no subscription row (currently shows 'Basic (Free)')",
                "Non-admin cannot call the admin entitlement endpoint (403); alembic single head; backend suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_subscriptions.py -q\npython scripts/check_premium.py <signed-in-email>\nxcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-283."),
        ),
    },
    {
        "summary": "TIME-283: Timezone follows the device, and 'today' means local",
        "labels": ["backend", "ios", "timezone", "bug"],
        "description": doc(
            h2("Goal"), p("The app must track whatever timezone the device is in, anywhere in the world, the same way the iPhone's own clock updates when a plane lands — this is not a Japan-specific fix. Today it does not. iOS pushes TimeZone.current in exactly one place — a .task{} on MainTabView — which fires once when the view is first created and never again; there is no scenePhase observer and no NSSystemTimeZoneDidChange observer anywhere in ios/, and the PATCH is wrapped in try? so failures vanish silently. Resuming the app from background after a flight leaves the stored profile timezone wrong forever. Underneath that, several backend paths still compute 'today' from UTC regardless of the stored timezone, and all Celery beat jobs fire at fixed UTC hours for every user."),
            divider(), h2("Scope"), bullet_list([
                "iOS: extract syncDeviceTimezone out of MainTabView into a TimezoneSyncService; fire on launch, on scenePhase -> .active, and on NSSystemTimeZoneDidChange (the same system notification that updates the iPhone clock on landing); send whatever TimeZone.current reports, with no allowlist or special-casing of any region; only PATCH when the identifier actually changed; retry with backoff and log failures instead of swallowing them",
                "Backend: use the stored profile timezone for day boundaries in api/v1/timeline.py (both /timeline/today and /timeline/today/plan, removing the abs(for_date - utc_today) <= 1 fudge), repositories/task_repository.py, repositories/meal_repository.py, repositories/sleep_wake_repository.py, services/insights_service.py, services/morning_replan.py",
                "Reuse the already-correct idiom from api/v1/now.py and api/v1/activity.py rather than reimplementing zoneinfo conversion",
                "Celery beat: run the morning/learning/evening check-in jobs hourly and filter per user on 'is it their local 8am/10am/9pm', mirroring the existing scan_and_push sweep shape",
                "Correctness across the full IANA range, not just whole-hour western zones: cover the date-line side (Asia/Tokyo, Australia/Sydney, Pacific/Auckland), half-hour and 45-minute offsets (Asia/Kolkata, Asia/Kathmandu), zones with no DST (Africa/Lagos, Asia/Shanghai), and DST transition days",
                "Document in known_issues.md that historical DailyActivity.day rows carry no timezone, so past patterns shift when a user relocates",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No per-row timezone stamping or backfill of historical activity data",
                "No timezone header on every API request — the stored profile timezone stays the source of truth",
                "No change to the capture endpoint's existing per-request user_timezone field",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/App/MainTabView.swift, TimeSense/Core/Time/TimezoneSyncService.swift (new)",
                "backend app/api/v1/timeline.py, app/repositories/task_repository.py, app/repositories/meal_repository.py, app/repositories/sleep_wake_repository.py, app/services/insights_service.py, app/services/morning_replan.py, app/workers/celery_app.py, app/workers/notification_tasks.py",
                "backend tests/test_timeline.py, tests/test_timezone_boundaries.py (new)",
                "docs/project_memory/known_issues.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Changing the device timezone to ANY zone and returning the app to the foreground PATCHes the new identifier within one app resume; no PATCH when unchanged",
                "Day-boundary tests are parametrized over Asia/Tokyo, Asia/Shanghai, Australia/Sydney, Africa/Lagos, Asia/Kolkata and America/New_York — /timeline/today, meals, sleep and task day bounds return that zone's day in every case, not the UTC day",
                "Flying between any two zones is handled by the same code path; no region is special-cased",
                "Check-in notifications fire at the user's local hour, not a fixed UTC hour",
                "New tests pin a non-UTC timezone across every changed repository; backend suite green; iOS BUILD SUCCEEDED",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_timeline.py tests/test_timezone_boundaries.py tests/test_now.py -q\nxcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-284."),
        ),
    },
    {
        "summary": "TIME-284: Baseline task library — types with typical minutes and difficulty",
        "labels": ["backend", "recommendations", "estimation"],
        "description": doc(
            h2("Goal"), p("Task duration estimates are seeded from a 15-entry category table, and infer_category() returns the catch-all 'general' for most real task titles. There is also no concept of task difficulty anywhere — required energy is derived purely from estimated duration (>=45 min counts as high energy). Build a proper baseline library of common task types, each carrying a typical duration AND a difficulty level, so a brand-new user gets sensible estimates before any learning happens and difficulty becomes a first-class field."),
            divider(), h2("Scope"), bullet_list([
                "New app/services/task_library.py: ~120-150 task types, each (type_key, display_name, category, typical_minutes, difficulty: light|moderate|deep, keywords)",
                "Ordered specific-to-general keyword matcher returning (type_key, typical_minutes, difficulty)",
                "Task model gains task_type and difficulty nullable columns + Alembic migration",
                "Supersede DEFAULT_DURATIONS in services/task_duration.py; keep infer_category during the transition so nothing breaks mid-batch",
                "Title-fixture test table asserting common phrasings resolve to a real type rather than the catch-all",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No LLM classification yet — that is TIME-285",
                "No change to the learning/EWMA logic yet — that is TIME-286",
                "No user-facing type picker yet — that is TIME-287",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/task_library.py (new), app/services/task_duration.py, app/models/task.py, app/schemas/task.py",
                "backend migrations/versions/<new>.py, tests/test_task_library.py (new)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A fixture list of realistic task titles resolves to specific types; catch-all rate measurably below the current behaviour",
                "Every library entry has a typical_minutes and a difficulty; no duplicate type_keys",
                "Task.task_type and Task.difficulty exist and migrate cleanly; alembic single head; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_task_library.py tests/test_task_duration.py -q"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-285."),
        ),
    },
    {
        "summary": "TIME-285: Classify captured tasks into a library type and difficulty",
        "labels": ["backend", "capture", "llm", "estimation"],
        "description": doc(
            h2("Goal"), p("TIME-284 introduced task types and difficulty, but nothing populates them. Wire classification into every path that creates a task — capture, Notion import, email confirm, Slack confirm, manual create — so the baseline library actually applies, with a deterministic fallback whenever the LLM is unavailable or returns an unknown key."),
            divider(), h2("Scope"), bullet_list([
                "CaptureService LLM prompt gains task_type (constrained to library keys) and difficulty; output validated against the library and rejected to the fallback if unknown",
                "Keyword matcher from TIME-284 is the deterministic fallback, matching the existing pattern where the rule-based parser fills LLM gaps",
                "Set task_type/difficulty in api/v1/capture.py, api/v1/tasks.py, and the shared services/task_autoschedule.py helper used by Notion and email imports",
                "Existing tasks stay null and fall back to the library seed at read time — no backfill required",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No user-facing override UI (TIME-287)",
                "No change to duration learning (TIME-286)",
                "No re-classification of historical tasks",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/capture_service.py, app/api/v1/capture.py, app/api/v1/tasks.py, app/services/task_autoschedule.py, app/services/notion_service.py, app/services/email_service.py",
                "backend tests/test_capture.py, tests/test_task_classification.py (new)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A captured task has a non-null task_type and difficulty on every creation path",
                "With the LLM unavailable or returning an invalid key, the keyword fallback still yields a valid type",
                "Suite green including the existing capture tests",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_capture.py tests/test_task_classification.py tests/test_task_autoschedule.py -q"),
            divider(), h2("Dependencies"), p("TIME-284."), divider(), h2("Next Ticket"), p("TIME-286."),
        ),
    },
    {
        "summary": "TIME-286: Duration learning keyed on task type, with raw observations",
        "labels": ["backend", "estimation", "learning", "bug"],
        "description": doc(
            h2("Goal"), p("The user reports that nearly every task is estimated at 23 minutes regardless of what it is. Root cause: learned durations are stored per coarse category, infer_category() sends most titles to the 'general' catch-all, and the iOS prompt offers only three answers (~15 / ~30 / ~1 hour). An EWMA with alpha 0.3 over 15, 30, 30 produces exactly 15 -> 20 -> 23, and from then on every unclassified task inherits that single number. Raw observations are never persisted, only the blended value, so nothing can be recomputed. Rebuild the learning on top of the TIME-284 library."),
            divider(), h2("Scope"), bullet_list([
                "New TaskDurationObservation model (user_id, task_id, task_type, estimated_minutes, actual_minutes, observed_at) + migration — persist the raw actuals that are currently discarded",
                "TaskDurationRepository learns per task_type instead of the coarse category; never learns on the catch-all bucket, so an unclassified task always falls back to the library seed",
                "Replace the naked EWMA with a confidence-weighted blend toward the baseline: estimate = (n*learned + k*baseline) / (n + k), k approximately 3, so a single answer cannot drag every estimate",
                "Discard existing task_duration_estimates rows for the 'general' category (they are the bug); migrate typed categories across to their library type",
                "Regression test replaying the 15/30/30 sequence and asserting unrelated tasks keep their own library seed",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to the iOS feedback UI (TIME-287)",
                "No per-title or embedding-based similarity — type-level granularity only",
                "No retroactive re-estimation of already-scheduled tasks",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/models/task_duration.py, app/repositories/task_duration_repository.py, app/services/task_duration_service.py",
                "backend migrations/versions/<new>.py, tests/test_task_duration.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Recording 15, 30, 30 for one task type leaves every other type's estimate at its library seed",
                "Raw observations are persisted and readable per user and type",
                "An unclassified task never returns a learned value; it returns the library seed",
                "The blend converges toward the observed mean as sample count grows; alembic single head; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_task_duration.py tests/test_tasks.py -q"),
            divider(), h2("Dependencies"), p("TIME-284, TIME-285."), divider(), h2("Next Ticket"), p("TIME-287."),
        ),
    },
    {
        "summary": "TIME-287: Real duration feedback on iOS (replace the three buttons)",
        "labels": ["ios", "android", "estimation", "ux"],
        "description": doc(
            h2("Goal"), p("The 'how long did that take?' prompt offers only ~15 min, ~30 min and ~1 hour. Those three coarse answers are what feed the EWMA that produced the 23-minute problem, and they cannot express 'it took 5 minutes' or 'it took two hours'. Give the user a real answer, and add a passive path that captures an actual duration without asking at all."),
            divider(), h2("Scope"), bullet_list([
                "Replace the three-button confirmationDialog with a minute entry pre-filled from the current estimate, with quick presets retained as shortcuts",
                "Add a start/finish timer path on the recommended action so completing a started task records an actual duration passively",
                "Keep a clear dismiss option — capturing durations must never become another chore",
                "Show the task's library type with an override picker, writing the correction back so classification improves",
                "Android parity where the JDK is available",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No background/automatic time tracking",
                "No web companion change",
                "No new backend learning logic beyond what TIME-286 already provides",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Features/Now/NowView.swift, NowViewModel.swift",
                "android app/src/main/java/com/timesense/app/features/now/*",
                "backend app/api/v1/tasks.py (accept a task_type correction on the duration-feedback endpoint)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "An arbitrary minute value can be submitted and reaches the duration-feedback endpoint",
                "Starting and finishing a task records an actual duration with no prompt",
                "Dismissing the prompt records nothing and does not re-ask immediately",
                "iOS BUILD SUCCEEDED; Android assembleDebug succeeds when a JDK is present",
            ]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO\ncd backend && pytest tests/test_tasks.py -q"),
            divider(), h2("Dependencies"), p("TIME-286."), divider(), h2("Next Ticket"), p("TIME-288."),
        ),
    },
    {
        "summary": "TIME-288: One EnergyService — energy is a recovery budget that depletes",
        "labels": ["backend", "recommendations", "energy", "bug"],
        "description": doc(
            h2("Goal"), p("The energy signal is backwards and internally inconsistent. Two different values exist: the scorer derives energy from sleep only and hard-codes 'medium' whenever there is no sleep sample, so activity never reaches scoring at all; the display layer separately computes energy from activity, where 30+ minutes of exercise or 8000+ steps reads as HIGH energy. The result the user reported is that a busy day produces 'high energy' and a demanding task is recommended in the evening. Being busy all day should DEPLETE energy, not raise it. Collapse both paths into one service with a recovery-budget model."),
            divider(), h2("Scope"), bullet_list([
                "New app/services/energy_service.py as the single source of energy for the scorer, the Now context cards, and the Why sheet",
                "Last night's sleep sets a morning energy budget; the budget depletes across the day with hours awake, meeting and deep-work minutes already spent, and cumulative activity (steps, exercise minutes, workout sessions)",
                "Apply a circadian shape: post-lunch dip and evening decline, so identical sleep does not yield identical energy at 9am and 8pm",
                "Unify the vocabulary on low | medium | high — the display layer's 'moderate' would KeyError against the engine's rank map and only survives today because the engine never sees it",
                "Graceful degradation with no HealthKit data: time-since-typical-wake plus the circadian shape, instead of hard-coding 'medium'",
                "Re-express the existing poor-sleep deep-work suppression penalty against the new budget, preserving the build spec's intent",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No self-report check-in yet (TIME-289)",
                "No change to how required energy is derived from a task (TIME-290)",
                "No new HealthKit reads — use the data already ingested",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/energy_service.py (new), app/services/recommendation/context_builder.py, app/services/recommendation_explainer.py, app/api/v1/now.py, app/services/recommendation/scoring/penalties.py",
                "backend tests/test_energy_service.py (new), tests/test_now.py, tests/test_explanation_reasoning.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A day with high step count, several meetings and many hours awake yields LOW energy at 8pm",
                "Good sleep, early morning, little activity yields HIGH energy",
                "The Why sheet and the scorer report the same energy value and the same vocabulary",
                "With no sleep and no activity data the service still returns a reasoned value rather than a hard-coded constant; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_energy_service.py tests/test_now.py tests/test_now_recommendation.py tests/test_explanation_reasoning.py -q"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-289."),
        ),
    },
    {
        "summary": "TIME-289: Energy check-in that overrides the inferred value",
        "labels": ["backend", "ios", "energy", "ux"],
        "description": doc(
            h2("Goal"), p("Inferred energy will always be approximate. Let the user correct it in one tap, have that correction actually drive recommendations for a bounded window, and use the gap between reported and inferred energy to calibrate the TIME-288 model — without turning the app into another thing to maintain."),
            divider(), h2("Scope"), bullet_list([
                "New EnergyCheckIn model (user_id, reported: low|medium|high, reported_at) + migration",
                "POST /api/v1/energy/checkin; EnergyService prefers a recent check-in over the inferred value for a bounded window of roughly 4 hours",
                "The Now energy card becomes tappable with a three-option self-report; the Why sheet attributes the value to the user when a check-in is active",
                "Optional morning prompt gated on the existing notification_mode so only active_coach users are ever asked, reusing NotificationService's check-in machinery",
                "Persist reported-vs-inferred pairs so the curve can be calibrated later",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No automatic calibration of the model in this ticket — collect the pairs first",
                "No prompt for gentle or balanced notification modes",
                "No Android or web surface in this ticket",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/models/energy_checkin.py (new), app/api/v1/energy.py (new), app/services/energy_service.py, app/services/notification_service.py",
                "backend migrations/versions/<new>.py, tests/test_energy_checkin.py (new)",
                "ios TimeSense/Features/Now/NowView.swift, NowViewModel.swift",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A check-in overrides the inferred energy for the window and then expires cleanly",
                "The Why sheet says the value came from the user while a check-in is active",
                "Gentle and balanced users are never prompted; alembic single head; suite green; iOS BUILD SUCCEEDED",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_energy_checkin.py tests/test_energy_service.py tests/test_notifications.py -q\nxcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-288."), divider(), h2("Next Ticket"), p("TIME-290."),
        ),
    },
    {
        "summary": "TIME-290: Required energy comes from difficulty, and adapts per user",
        "labels": ["backend", "recommendations", "energy"],
        "description": doc(
            h2("Goal"), p("task_required_energy() decides a task needs high energy purely because it is 45 minutes or longer, and low energy because it is 15 minutes or shorter. A 90-minute podcast is therefore 'high energy' and a 10-minute difficult phone call is 'low energy'. Now that TIME-284 gives tasks a real difficulty, use it — and let the mapping adapt to how the individual user actually performs."),
            divider(), h2("Scope"), bullet_list([
                "task_required_energy reads Task.difficulty; duration becomes a secondary tiebreak only",
                "Fall back to the library difficulty for the task's type when the task itself has none, and only then to duration",
                "Per-user adjustment sourced from the TIME-292 adaptation profile: if a user reliably completes deep tasks in the evening, shift their personal difficulty-to-energy mapping rather than fighting them",
                "Keep the mapping neutral below a sample floor so a new user is not scored on noise",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to how energy itself is computed (TIME-288)",
                "No user-facing difficulty editor beyond the type override shipped in TIME-287",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/recommendation/candidates/common.py, app/services/recommendation/candidates/task_candidates.py, app/services/recommendation/scoring/penalties.py",
                "backend tests/test_recommendation_energy.py (new), tests/test_now_recommendation.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A long, light task no longer requires high energy; a short, deep task does",
                "With no difficulty available the behaviour degrades to the previous duration heuristic",
                "The per-user adjustment stays neutral below its sample floor; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_recommendation_energy.py tests/test_now_recommendation.py tests/test_recommendations.py -q"),
            divider(), h2("Dependencies"), p("TIME-284, TIME-288, TIME-292."), divider(), h2("Next Ticket"), p("TIME-291."),
        ),
    },
    {
        "summary": "TIME-291: Repair the location signal end-to-end",
        "labels": ["ios", "backend", "location", "bug"],
        "description": doc(
            h2("Goal"), p("Location no longer influences recommendations. Six stacked breaks: permission is requested only from Settings > Places, which onboarding never routes users to; geofences register only under authorizedAlways, so 'While Using' silently disables the whole path; the current place is posted only for manually saved places, so a user with zero saved places never reports a location at all; coordinates are never persisted, so errand candidates hit LOCATION_DATA_MISSING unless the reported place name exactly matches a saved place; the signal goes stale after six hours with nothing to refresh it; and location_fit is a flat constant for every non-errand candidate. Repair the chain."),
            divider(), h2("Scope"), bullet_list([
                "iOS: ask for location during onboarding with a clear rationale, and seed Home (and optionally Work) as UserPlace rows so geofences exist from day one",
                "Support authorizedWhenInUse instead of doing nothing: post the current place on foreground and on significant-location change; reserve geofence registration for authorizedAlways",
                "Persist coordinates on UserLocationState (+ migration) so the recommendation location service stops returning coordinates=None and errand candidates can be travel-checked",
                "Refresh the current place before the six-hour staleness cutoff rather than letting the signal disappear",
                "Keep the existing privacy stance: consent-gated on location_tracking, one upserted current row, no raw location history",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No location history table",
                "No change to location_fit scoring itself — that lands in TIME-293",
                "No Android location work in this ticket",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Core/Location/LocationService.swift, TimeSense/Features/Onboarding/OnboardingView.swift, TimeSense/Features/Settings/PlacesSettingsView.swift",
                "backend app/models/user_location_state.py, app/repositories/user_location_repository.py, app/services/recommendation/location_service.py, app/api/v1/location.py",
                "backend migrations/versions/<new>.py, tests/test_location.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A user who grants While Using still reports a current place",
                "A user with no manually saved places still has a location signal after onboarding",
                "UserLocationState carries coordinates; errand candidates no longer report LOCATION_DATA_MISSING when a place is known",
                "The signal refreshes before six hours; alembic single head; suite green; iOS BUILD SUCCEEDED",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_location.py tests/test_recommendations.py -q\nxcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-292."),
        ),
    },
    {
        "summary": "TIME-292: UserAdaptationProfile rollup + nightly aggregation",
        "labels": ["backend", "learning", "celery"],
        "description": doc(
            h2("Goal"), p("Asked how TimeSense learns each user's habits, the honest answer today is: a duration EWMA, an acceptance rate keyed on action type, and two read-only 'what we learned' surfaces that feed nothing back into scoring. Everything else is recomputed live per request over 28 or 30 day windows, which is too expensive to consult on every recommendation. Create the first table whose purpose is adaptation, and a nightly job that fills it."),
            divider(), h2("Scope"), bullet_list([
                "New UserAdaptationProfile model + migration: completion rate by hour-of-day and weekday; acceptance rate by task category and by action_type; estimate accuracy (predicted vs actual) by task type; energy-at-completion distribution; typical wake, start and wind-down times",
                "Nightly Celery job following the existing generate_weekly_insights pattern, plus beat registration",
                "Repository read path cheap enough to consult on every /now request",
                "Backfill from existing recommendation_events, recommendation_feedback, tasks and activity data on first run",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to scoring in this ticket — TIME-293 consumes the profile",
                "No new user-facing surface; the existing Learned Patterns screen is untouched here",
                "No third-party analytics pipeline",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/models/user_adaptation_profile.py (new), app/repositories/user_adaptation_repository.py (new), app/services/user_adaptation_service.py (new), app/workers/adaptation_tasks.py (new), app/workers/celery_app.py",
                "backend migrations/versions/<new>.py, tests/test_user_adaptation.py (new)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "The nightly job produces a profile row per active user and is idempotent when re-run",
                "Every metric is null (not zero) below its sample floor, so downstream scoring can stay neutral",
                "Reading a profile is a single indexed query; alembic single head; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_user_adaptation.py tests/test_insights.py -q"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-293."),
        ),
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Scope"), bullet_list([
                ...
                "user_preference_fit computed from per-category acceptance in the profile, extending the existing action_type logic rather than replacing it, and keeping its five-sample floor",
                "location_fit computed from real proximity and travel feasibility for any task with a resolvable place, using the already-live maps provider",
                "Every fit stays neutral at 0.5 below its sample floor so a new user is never scored on noise",
                "Score-differentiation regression tests: two tasks differing in exactly one of these four factors must now receive different scores",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to the weight constants themselves — the spec's formula stands",
                "No per-user learned weights",
                "No LLM involvement in selection",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/recommendation/candidates/task_candidates.py, candidates/common.py, types.py, app/services/recommendation/feedback/apply_feedback.py",
                "backend tests/test_score_differentiation.py (new), tests/test_now_recommendation.py, tests/test_recommendations.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Two otherwise identical tasks differing only in routine fit, context fit, preference fit or location fit receive different scores",
                "A user with no history still gets neutral values for all four, and ranking degrades to the current urgency/importance behaviour",
                "No regression in the existing recommendation suites; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_score_differentiation.py tests/test_now_recommendation.py tests/test_recommendations.py tests/test_now.py -q"),
            divider(), h2("Dependencies"), p("TIME-284, TIME-291, TIME-292."), divider(), h2("Next Ticket"), p("TIME-294."),
        ),
    },
    {
        "summary": "TIME-294: Backend — pinned recommendation and swap event on disagree",
        "labels": ["backend", "recommendations", "feedback"],
        "description": doc(
            h2("Goal"), p("When the user disagrees with a recommendation they pick a reason, and that is the end of it — the reason is write-only apart from lengthening a demote window. The user wants to say what they would rather do instead, and have TimeSense honour it and learn from it. Add the backend half: record the rejected/chosen pair with its context, and pin the chosen task as the Now recommendation."),
            divider(), h2("Scope"), bullet_list([
                "New RecommendationSwap model + migration: user_id, rejected_task_id, reason, chosen_task_id, context snapshot (part_of_day, energy, location_category, free_minutes), created_at",
                "POST /api/v1/recommendations/swap records the swap and pins the chosen task as the Now pick for a bounded window of roughly three hours, or until it is completed or dismissed",
                "The pin is honoured in now._engine_rank_tasks ahead of scoring and echoed on /now so the client can label it",
                "Additive only — the existing disagree-plus-reason write path is unchanged",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No learning from the swap yet (TIME-296)",
                "No client change (TIME-295)",
                "No permanent priority change to the chosen task",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/models/recommendation_swap.py (new), app/repositories/recommendation_swap_repository.py (new), app/api/v1/recommendations.py, app/api/v1/now.py, app/schemas/recommendation.py",
                "backend migrations/versions/<new>.py, tests/test_recommendation_swap.py (new)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Posting a swap makes the chosen task the Now recommendation on the next fetch",
                "The pin expires after its window and on completion or dismissal",
                "The swap row captures the full context snapshot; alembic single head; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && alembic heads && alembic upgrade head && pytest tests/test_recommendation_swap.py tests/test_feedback.py tests/test_now.py -q"),
            divider(), h2("Dependencies"), p("None."), divider(), h2("Next Ticket"), p("TIME-295."),
        ),
    },
    {
        "summary": "TIME-295: iOS — pick a replacement task from Today after disagreeing",
        "labels": ["ios", "recommendations", "ux"],
        "description": doc(
            h2("Goal"), p("Complete the disagree flow on device: after choosing why a recommendation is wrong, let the user pick which of today's tasks they would rather do, and see it become the recommendation immediately."),
            divider(), h2("Scope"), bullet_list([
                "After the existing 'Why not this one?' reason dialog, present a sheet listing today's actionable tasks, reusing GET /timeline/today/plan and filtering to tasks (calendar blocks are read-only and already excluded as candidates)",
                "Selecting a task calls POST /api/v1/recommendations/swap and refreshes Now, which then shows the chosen task labelled as the user's pick",
                "'Just skip' preserves exactly today's behaviour — reason recorded, no swap",
                "Reuse TodayViewModel's decoding and the shared taskCategoryStyle so the rows look like Today",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No Android or web parity in this ticket",
                "No reordering of the Today list itself",
                "No new task creation from this sheet",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Features/Now/NowView.swift, NowViewModel.swift, TimeSense/Features/Today/TodayViewModel.swift",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Disagree, pick a reason, pick a replacement — Now shows the replacement, labelled as the user's choice",
                "Cancelling at any step leaves current behaviour unchanged",
                "The sheet lists only actionable tasks, never calendar blocks; iOS BUILD SUCCEEDED",
            ]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-294."), divider(), h2("Next Ticket"), p("TIME-296."),
        ),
    },
    {
        "summary": "TIME-296: Learn from swaps and make the disagree reason count",
        "labels": ["backend", "learning", "recommendations"],
        "description": doc(
            h2("Goal"), p("A swap is the richest feedback the product can collect — it is a paired preference in a known context, not just a rejection. Today the disagree reason is read in exactly one place, to choose between a three-hour and a twenty-four-hour demote window. Feed both the swap pairs and the reasons into scoring."),
            divider(), h2("Scope"), bullet_list([
                "build_feedback_summary joins RecommendationSwap alongside recommendation_events: down-weight the rejected task's type in that context, up-weight the chosen one",
                "Repeated swaps at the same part of day emit a reason code (for example USER_PREFERS_THIS_TYPE_NOW) consumed by scoring/penalties.py",
                "Map the disagree reasons to real adjustments: wrong_time to a time-of-day penalty, too_big to a difficulty and duration penalty in the current energy state, not_priority to importance recalibration, not_relevant to the existing longer demote",
                "Sample floors throughout so a single swap cannot flip the model",
                "Surface newly learned preferences through the existing /recommendations/learned endpoint",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No learned scoring weights — the spec's weight formula stands",
                "No cross-user or global model",
                "No new client surface beyond the existing Learned Patterns screen",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend app/services/recommendation/feedback/build_summary.py, feedback/apply_feedback.py, app/services/recommendation/scoring/penalties.py, app/services/learned_preferences_service.py, app/repositories/recommendation_feedback_repository.py",
                "backend tests/test_swap_learning.py (new), tests/test_feedback.py",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Repeated swaps of the same type pairing in the same context measurably change ranking",
                "Each disagree reason produces a distinct, testable scoring effect",
                "Below the sample floor, ranking is unchanged from TIME-293 behaviour; suite green",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_swap_learning.py tests/test_feedback.py tests/test_now_recommendation.py tests/test_recommendations.py -q"),
            divider(), h2("Dependencies"), p("TIME-292, TIME-293, TIME-294."), divider(), h2("Next Ticket"), p("TIME-297."),
        ),
    },
    {
        "summary": "TIME-297: Learning and adaptation spec + project memory close-out",
        "labels": ["docs", "project-memory"],
        "description": doc(
            h2("Goal"), p("Answer, in the repository rather than in chat, how TimeSense learns each user's habits and adapts to them — and be equally explicit about what it still does not learn. Close out the batch in project memory so the next session resumes from files, not conversation."),
            divider(), h2("Scope"), bullet_list([
                "New docs/architecture/learning_and_adaptation_spec.md: every signal, where it is stored, how it is aggregated, what it changes in the score, and the current known limits",
                "Update implementation_log.md, phase_status.md, change_summary.md, context_summary.md and CHANGELOG.md for TIME-282 through TIME-296",
                "decision_log.md entries: energy as a depleting recovery budget; difficulty as a first-class task field; a nightly adaptation rollup in preference to live recomputation; swaps as paired training signal",
                "known_issues.md entries for what this batch deliberately left open, including the untimezoned historical activity rows and the still-stubbed purchase flow",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No code changes",
                "No new product decisions — this records decisions already taken",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "docs/architecture/learning_and_adaptation_spec.md (new)",
                "docs/project_memory/*.md, CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "The spec describes the full signal-to-score-to-learn loop as actually implemented after this batch",
                "context_summary.md states the current build state and the next recommended step",
                "Every ticket in the batch appears in the implementation log with its Jira key",
            ]),
            divider(), h2("Verification"), code_block("git diff --stat docs/ CHANGELOG.md"),
            divider(), h2("Dependencies"), p("TIME-282 through TIME-296."), divider(), h2("Next Ticket"), p("TIME-298."),
        ),
    },
    # ── TIMER FOLLOW-UPS (device feedback on TIME-287, 2026-08-29) ───────────
    {
        "summary": "TIME-298: The task timer persists across navigation and app restarts, and visibly runs",
        "labels": ["ios", "bug", "estimation", "ux"],
        "description": doc(
            h2("Goal"), p("Two defects in the TIME-287 timer, both reported from device use. (1) After tapping Start timer the label reads 'Timing…' with nothing moving, so there is no evidence the timer is alive — elapsedMinutes returns nil below one minute because plausibleMinutes requires >= 1, and the ticker only fires every 30s. (2) Navigating to another tab and back loses the running timer: QuickActionRow holds `agreed` and `timing` as @State, so the view being recreated resets them, and startedAt is an in-memory dictionary on the view model so it does not survive app termination either. The user then has to re-Agree and re-Start, and the elapsed time is lost. A timer that forgets is worse than no timer, because the user believes work is being measured when it is not."),
            divider(), h2("Scope"), bullet_list([
                "Persist the running timer outside view state, keyed by task id, so it survives navigation AND app termination (UserDefaults; the app group suite is already used for widget snapshots)",
                "Restore the running state when the Now screen appears: if a timer is active for the current recommendation, show it running without requiring the user to Agree or Start again",
                "Show live elapsed time including seconds (mm:ss) so it is visibly moving; tick every second while visible and stop ticking when the view disappears",
                "Derive elapsed from the stored START TIMESTAMP, never from an accumulated counter, so backgrounding the app cannot drift or under-count",
                "Keep the existing plausibility guard on the SUBMITTED value (1..480 min) while allowing the DISPLAY to show sub-minute time",
                "Clear the stored timer when the task is completed, dismissed, or the user stops it",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No server-side timer state — the timer is a client concern and must work offline",
                "No Live Activity / Dynamic Island (worth considering later, out of scope here)",
                "No overrun notification — that is TIME-299",
                "No Android or web equivalent (the duration prompt is iOS-only, as it was before)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Features/Now/NowViewModel.swift (persisted timer store, elapsed seconds)",
                "ios TimeSense/Features/Now/NowView.swift (QuickActionRow restore + live label)",
                "ios TimeSense/Core/Widgets/ (reuse the existing shared-defaults suite if appropriate)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Starting a timer and switching tabs and back shows it STILL RUNNING, with the correct elapsed time, and no need to Agree or Start again",
                "Force-quitting and reopening the app shows the timer still running with time that reflects real elapsed wall-clock",
                "The label changes at least once per second while visible, so it is obviously live",
                "Completing the task clears the timer and pre-fills the duration sheet with the measured value",
                "A timer left running absurdly long still cannot submit an implausible duration",
                "iOS BUILD SUCCEEDED",
            ]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO\n# then on device: start a timer, switch tabs, force-quit, reopen — it should still be running"),
            divider(), h2("Dependencies"), p("TIME-287."), divider(), h2("Next Ticket"), p("TIME-299."),
        ),
    },
    {
        "summary": "TIME-299: Prompt when a timed task runs well past its estimate",
        "labels": ["ios", "notifications", "estimation", "ux"],
        "description": doc(
            h2("Goal"), p("A running timer has no end condition, so a task the user finished without stopping the timer keeps counting and eventually submits nothing useful (the plausibility guard discards it) — or worse, submits a wildly inflated duration that poisons the learned estimate for that task type. Every task has an estimate (TIME-284's library guarantees one), so once a timer passes its estimate by a clear margin the assistant should ask whether the task is done rather than silently counting. This also turns a dead timer into a useful signal: 'this took much longer than we thought' is exactly what the duration learning wants to hear."),
            divider(), h2("Scope"), bullet_list([
                "When a timer starts, schedule a LOCAL notification for (start + estimated_minutes + OVERRUN_GRACE, 30 min) asking whether the task is finished",
                "Local rather than push: the timer is client-side state, it must work offline, and it must not depend on APNs being configured",
                "Cancel the scheduled notification when the task is completed, the timer is stopped, or the recommendation changes away from that task",
                "In-app equivalent: when the user opens Now with a timer already past its estimate, surface the same prompt inline rather than relying on them having seen the notification",
                "Tapping the prompt opens the duration sheet pre-filled with the measured time so completing it is one tap",
                "Nudge ONCE per timer, not repeatedly — the product rule is explicitly no nagging",
                "Fall back to the library baseline duration when a task somehow has no estimate",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No auto-completion of the task — the assistant asks, the user decides (same principle as replans and calendar writes)",
                "No auto-stopping of the timer",
                "No server-side scheduling or APNs dependency",
                "No repeated escalating reminders",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Features/Now/NowViewModel.swift, NowView.swift",
                "ios TimeSense/Core/ (notification scheduling helper)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Starting a timer on a task estimated at N minutes schedules exactly one notification for N+30 minutes later",
                "Completing the task, or stopping the timer, cancels the pending notification",
                "Reopening the app with an overrunning timer shows an in-app prompt without needing the notification",
                "Accepting the prompt opens the duration sheet pre-filled with the measured duration",
                "Only one nudge is delivered per timer run",
                "iOS BUILD SUCCEEDED",
            ]),
    ...
    {
        "summary": "TIME-316: Complete any task in Today and record how long it took",
        "labels": ["ios", "backend", "today", "learning", "duration"],
        "description": doc(
            h2("Goal"), p("Sometimes the user gets an opportunity to do a task that was NOT the recommendation at that moment — a better fit for their mood, or they knew something TimeSense did not. They need to mark it complete and say how long it took, without that becoming a chore. The experience already exists and is good, but it is reachable from exactly one place: the Now screen's recommended task. TodayViewModel.markDone only sends PATCH status=done — it never asks how long and never stops a running timer, so completing anything from Today teaches the duration estimator nothing and loses any timed measurement. Separately, completing a non-recommended task is completely invisible to learning: recommendation_events knows what was recommended and when, tasks knows a status changed, and nothing joins them."),
            divider(), h2("Scope"), bullet_list([
                "Swiping Done on ANY task in Today asks how long it took, exactly as the Now screen already does",
                "It asks only while still learning that task type, reusing the existing server-side gate — most completions stay one tap",
                "The duration flow and its sheet are extracted so Now and Today share one copy instead of duplicating it",
                "Completing a task from Today stops a running timer for it, and a timed figure pre-fills the sheet",
                "An already-done row no longer re-fires completion when its circle is tapped",
                "tasks gains a real completed_at, stamped on the pending-to-done edge only",
                "Completing a task while a DIFFERENT one was recommended is recorded silently as an unpinned swap, so the engine learns the preference with no extra taps",
                "Completing the task that WAS recommended finally sets outcome=done, which no client has ever sent",
                "A stale swap pin is released when its task is completed",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No Android or web parity",
                "No retroactive duration entry for tasks completed earlier",
                "No 'why did you do this instead?' question — the learning is silent, with no extra decision",
                "No mood capture; mood exists nowhere in the product and adding it is its own decision",
                "No new timer-start affordance on Today",
                "No backfill of completed_at from updated_at, which would fabricate completion instants",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios TimeSense/Core/Duration/DurationFeedback.swift (new) and DurationFeedbackSheet.swift (new)",
                "ios TimeSense/Features/Today/TodayView.swift, TodayViewModel.swift",
                "ios TimeSense/Features/Now/NowView.swift, NowViewModel.swift",
                "backend app/services/task_completion_service.py (new)",
                "backend app/services/recommendation/swap_context.py (new)",
                "backend app/repositories/task_repository.py, recommendation_swap_repository.py, recommendation_event_repository.py",
                "backend migrations/versions (new: tasks.completed_at)",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Swiping Done on a Today task whose type is still being learned opens the duration sheet, pre-filled",
                "Swiping Done on a well-learned type completes in one tap with no sheet",
                "A task timed on Now and completed from Today pre-fills the timed figure AND stops the timer",
                "Tapping the circle on an already-done row does nothing",
                "completed_at is set on the pending-to-done edge and does not move on a repeat done or a later edit",
                "Completing the recommended task sets its impression outcome to done and writes no swap",
                "Completing a different task writes one unpinned swap naming both tasks, and never a disagree",
                "An unpinned swap is never returned by active_pin, so a finished task is never recommended next",
                "Marking many tasks done in a burst cannot teach a preference from a single recommendation",
                "A silent completion signal counts for less than a deliberate swap",
                "Learning failures never fail the completion itself",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_task_completion.py tests/test_completion_learning.py tests/test_recommendation_swap.py tests/test_swap_learning.py -q\ncd backend && alembic upgrade head\nxcodebuild test -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-286 (duration learning), TIME-287 (the duration sheet), TIME-294/296 (swap and swap learning), TIME-298 (the task timer)."),
            divider(), h2("Next Ticket"), p("(none)."),
        ),
    },
    {
    ...
    # ── STEPS & PREREQUISITES BATCH (planned 2026-09-15) ─────────────────────────────────────────────
    # Two user-requested features sharing one model: a task can have steps (one level), and a task can
    # wait for another ("B waits for A"). Ordered steps write those edges automatically. Plan settled
    # with the user; the 12 decisions land in decision_log.md with TIME-320.
    {
        "summary": "TIME-319: Capture's parse prompt always tells the LLM the user's local time is UTC",
        "labels": ["backend", "capture", "llm", "timezone", "bug"],
        "description": doc(
            h2("Goal"), p("capture_service.py builds the 'User's LOCAL date and time' line with ZoneInfo(user_timezone), but ZoneInfo is never imported in that module. The NameError is swallowed by the surrounding except, so every capture from a non-UTC user is parsed as if they lived in UTC: 'tonight' and 'tomorrow morning' are resolved against the wrong clock before the deterministic repairs run. Found while planning the steps batch, which edits the same prompt; fixing it first keeps that ticket honest."),
            divider(), h2("Scope"), bullet_list([
                "Import ZoneInfo in backend/app/services/capture_service.py",
                "Narrow the except around the local-time line so a coding error cannot be silently swallowed again (invalid timezone names still fall back to UTC)",
                "Regression test: the built prompt for America/New_York shows the New York local time, not UTC",
                "Regression test: an invalid timezone name still produces a prompt (UTC fallback), not an exception",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to the parse schema, prompt wording or max_tokens (TIME-325 owns those)",
                "No change to the deterministic date parser or implicit deadline resolver",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/services/capture_service.py",
                "backend/tests/test_capture_service.py (or the existing capture prompt test module)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "The parse prompt for a non-UTC user states that user's local date and time",
                "An unknown timezone string falls back to UTC without raising",
                "A NameError-class bug in that block would now fail tests instead of being swallowed",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q -k capture\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("None."),
            divider(), h2("Next Ticket"), p("TIME-320."),
        ),
    },
    {
        "summary": "TIME-320: Task steps and prerequisites data model, plus the task graph read layer",
        "labels": ["backend", "tasks", "steps", "prerequisites", "migration"],
        "description": doc(
            h2("Goal"), p("Tasks are flat rows today: no parent, no order, no dependency. This ticket adds the storage both features share and a single read layer that annotates any list of tasks with its graph facts, without N+1 queries. A task can be a step of exactly one parent (one level only). A prerequisite edge means 'this task waits for that one'; it is met when the prerequisite is done or cancelled. Nothing user-visible changes yet except additive response fields."),
            divider(), h2("Scope"), bullet_list([
                "Alembic migration off b2c3d4e5f0a1: tasks.parent_task_id (named self-FK, ON DELETE CASCADE, indexed, CHECK not self), tasks.position, tasks.steps_sequential (NOT NULL, server default false)",
                "New table task_prerequisites (task_id, prerequisite_task_id, user_id, origin 'sequence'|'manual', created_at); PK on the pair, index on prerequisite_task_id, CHECK not self",
                "TaskPrerequisite model with CHECKs in __table_args__ so SQLite tests enforce them; registered in models/__init__.py",
                "TaskPrerequisiteRepository: unmet_for(ids) in one query with titles, edges_for_user, add (idempotent), remove, rechain_steps(parent)",
                "TaskRepository bulk reads: get_many, step_counts, steps_for, next_position",
                "TaskGraphService: annotate(tasks), recommendable(tasks, info), responses(tasks) as the single TaskResponse serializer; a step inherits its parent's unmet prerequisites",
                "Additive TaskResponse fields: parent_task_id, parent_title, position, step_count, open_step_count, blocked_by, steps, suggested_parent",
                "Switch task, timeline and capture serialization to TaskGraphService.responses",
                "Record the 12 settled steps/prerequisites decisions in decision_log.md, including why this is not Projects",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No status cascade or auto-complete (TIME-321)",
                "No endpoints to create steps or prerequisites (TIME-321, TIME-322)",
                "No engine filtering or scheduling changes (TIME-323)",
                "No iOS, Android or web changes",
                "No ORM relationships for steps or edges (explicit queries only, to avoid async lazy loads)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/migrations/versions/<new>_add_task_steps_and_prerequisites.py",
                "backend/app/models/task.py, backend/app/models/__init__.py",
                "backend/app/repositories/task_repository.py, backend/app/repositories/task_prerequisite_repository.py (new)",
                "backend/app/services/task_graph.py (new)",
                "backend/app/schemas/task.py",
                "backend/app/api/v1/tasks.py, timeline.py, capture.py (serializer switch only)",
                "backend/tests/test_task_graph.py (new), backend/tests/test_privacy*.py (account deletion cascade)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "alembic upgrade head, downgrade -1, upgrade head all succeed against Postgres",
                "A task cannot be its own parent or its own prerequisite (DB CHECK, verified in tests)",
                "annotate() on N tasks issues a constant number of queries regardless of N",
                "blocked_by lists only prerequisites that are not done or cancelled; in_progress still blocks",
                "A step is reported blocked when its parent has an unmet prerequisite",
                "Existing task responses are unchanged apart from the new additive fields with empty defaults",
                "Deleting an account with parents, steps and edges still erases everything",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q tests/test_task_graph.py\ncd backend && pytest -q\ndocker compose up -d && cd backend && alembic upgrade head && alembic downgrade -1 && alembic upgrade head"),
            divider(), h2("Dependencies"), p("None (independent of TIME-319)."),
            divider(), h2("Next Ticket"), p("TIME-321 and TIME-322."),
        ),
    },
    {
        "summary": "TIME-321: Steps: attach, create, move, and auto-complete the parent",
        "labels": ["backend", "tasks", "steps"],
        "description": doc(
            h2("Goal"), p("Make steps real. Any standalone task becomes a parent when it gets its first step, a task can be attached to a parent later (the user often thinks of steps after the fact, or imports them), and the parent finishes by itself when its last step is done. Status effects live in TaskRepository because three code paths write done straight through it (TaskService, POST /recommendations/feedback, the Google Assistant webhook) and soft delete writes cancelled."),
            divider(), h2("Scope"), bullet_list([
                "TaskRepository._settle_graph on status change: last open step done completes the parent (completed_at stamped); reopening a step reopens a done parent (completed_at cleared); all steps cancelled leaves the parent a plain task; parent done closes open steps; parent cancelled cancels open steps; a cancelled step rechains sequence edges",
                "StepService.attach(user, task, parent, position, ordered): one level, ownership, open non-calendar parent; copies parent priority; rechains old and new groups; if the parent was auto-placed, unschedule it and place the first unblocked step",
                "StepService.create_steps(user, parent, drafts, sequential) using TaskDurationEstimator",
                "POST /tasks/{id}/steps; TaskCreate gains parent_task_id, steps (max 12), steps_sequential",
                "TaskUpdate gains parent_task_id (null removes from group) and position",
                "Duration prompt never asks for a parent; parent auto-completion records no completion learning; pins on cascade-closed steps are released",
                "Insights: completed counts only tasks without steps; created counts exclude steps",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No manual prerequisite endpoints (TIME-322)",
                "No engine filtering (TIME-323) and no Today plan nesting (TIME-324)",
                "No LLM step generation (TIME-325)",
                "No drag reordering; position is set by attach or by AI suggestion only",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/repositories/task_repository.py",
                "backend/app/services/step_service.py (new), backend/app/services/task_service.py",
                "backend/app/schemas/task.py, backend/app/api/v1/tasks.py",
                "backend/app/services/task_completion_service.py (pin release for cascaded steps)",
                "backend/tests/test_task_steps.py (new)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Completing the last open step via PATCH, recommendation feedback, or the assistant webhook completes the parent",
                "Reopening a step of a done parent reopens it and clears completed_at",
                "A step cannot be given steps and cannot be attached to a step (422)",
                "Attaching an existing task, moving it to another parent, and removing it from its group all rechain both groups",
                "Sequential steps are chained with origin='sequence' edges; unordered steps have none",
                "No duration prompt or learning event for an auto-completed parent",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q tests/test_task_steps.py\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-320."),
            divider(), h2("Next Ticket"), p("TIME-322."),
        ),
    },
    {
        "summary": "TIME-322: Prerequisites: 'Do this after' between any two tasks",
        "labels": ["backend", "tasks", "prerequisites"],
        "description": doc(
            h2("Goal"), p("Let a user say one task waits for another, across groups or between standalone tasks, steps included. One mechanism: the same task_prerequisites edge that ordered steps use, with origin='manual'. Cycles are refused so nothing can end up waiting on itself forever."),
            divider(), h2("Scope"), bullet_list([
                "PrerequisiteService.add/remove: both tasks must belong to the user (404), self is 400, calendar-sourced tasks 422, at most 10 prerequisites per task",
                "Cycle detection by BFS over edges_for_user; a would-be cycle returns 409",
                "POST /tasks/{id}/prerequisites and DELETE /tasks/{id}/prerequisites/{prereq_id}; responses use the annotated serializer",
                "Rechaining (TIME-321) never touches origin='manual' edges",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No engine or scheduling effect yet (TIME-323)",
                "No iOS picker (TIME-328)",
                "No hard block on completing a waiting task; waiting is advice to the engine, not a lock",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/services/prerequisite_service.py (new)",
                "backend/app/api/v1/tasks.py, backend/app/schemas/task.py",
                "backend/tests/test_task_prerequisites.py (new)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A waits-for edge between two of the user's tasks is created once, idempotently",
                "A cycle of any length is refused with 409 and nothing is written",
                "Another user's task id returns 404",
                "Cancelling a middle step rechains sequence edges but keeps manual ones",
                "blocked_by on the dependent task shows the prerequisite until it is done or cancelled",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q tests/test_task_prerequisites.py\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-320 (and TIME-321 for the rechain interaction test)."),
            divider(), h2("Next Ticket"), p("TIME-323."),
        ),
    },
    {
        "summary": "TIME-323: Recommendations and scheduling respect steps and prerequisites",
        "labels": ["backend", "recommendation-engine", "scheduling", "steps", "prerequisites"],
        "description": doc(
            h2("Goal"), p("TimeSense must never recommend a task that is waiting on an unfinished one, and never recommend a parent whose steps are still open (it recommends the next step instead, with its parent named). Several surfaces pick tasks without going through candidate gathering, so each is covered explicitly. Scheduling must not place a task before its prerequisites."),
            divider(), h2("Scope"), bullet_list([
                "candidate_gather.gather_candidate_tasks applies TaskGraphService.recommendable (covers /now, /now/why, /now/recommendation, proactive push)",
                "Bypassing surfaces: GET /recommendations, push_service time-block offer, google_assistant best task",
                "POST /recommendations/swap to a blocked task or a parent with open steps returns 409 naming what it waits for",
                "context_builder: a step scores with due = step.due_at or parent.due_at and priority = min(step, parent)",
                "autoschedule_task, capture auto-place and suggested-slot use not_before = latest scheduled_end of unmet prerequisites; no auto-placement while an unmet prerequisite is untimed",
                "Push and assistant text name the parent for a step ('Next for Renew passport: get photos')",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No score boost for tasks that unblock others (deferred, optional)",
                "No Today plan nesting (TIME-324)",
                "No change to scoring weights",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/services/recommendation/candidate_gather.py, context_builder.py",
                "backend/app/api/v1/recommendations.py, backend/app/api/v1/now.py, backend/app/api/v1/tasks.py",
                "backend/app/services/push_service.py, backend/app/integrations/google_assistant.py",
                "backend/app/services/task_autoschedule.py, backend/app/api/v1/capture.py",
                "backend/tests/test_now_engine.py, test_recommendation_swap.py, test_push_service.py, scheduling tests",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "/now never returns a blocked task or a parent with open steps, and does return the next step with parent_title",
                "A pinned task that becomes blocked is not forced first",
                "Legacy recommendations, the push offer and the assistant never pick a blocked task",
                "Swap to a blocked task returns 409",
                "A step with no due date scores with its parent's due date",
                "Auto-placement and suggested-slot never start before an unmet prerequisite's scheduled end",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q -k 'now or recommend or push or schedul or assistant'\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-321, TIME-322."),
            divider(), h2("Next Ticket"), p("TIME-324."),
        ),
    },
    {
        "summary": "TIME-324: Today plan nests steps under their parent",
        "labels": ["backend", "timeline", "today", "steps"],
        "description": doc(
            h2("Goal"), p("The Today plan should read as the user thinks of it: 'Renew passport, 1 of 3' with its steps underneath, not three unrelated rows. Blocked tasks stay visible with what they wait for, so iOS can dim them."),
            divider(), h2("Scope"), bullet_list([
                "GET /timeline/today/plan emits one kind:'task' entry per parent with task.steps nested (ordered by position); steps never appear as top-level entries",
                "Parents of timed steps are fetched even if the parent is untimed",
                "Cancelled steps are dropped from the nested list",
                "A group entry's start/end is its next open step's slot, else the parent's, else Anytime",
                "Every entry carries blocked_by, step_count, open_step_count",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "GET /timeline/today stays flat (dormant Android/web clients)",
                "No new entry kind, so older iOS builds render a group as a normal row",
                "No iOS rendering (TIME-327)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/api/v1/timeline.py, backend/app/schemas/timeline*.py",
                "backend/tests/test_timeline*.py",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A parent with three steps appears once, with three nested steps in order",
                "A step timed today whose parent is untimed still appears, under its parent",
                "Cancelled steps are not listed",
                "blocked_by titles are present on waiting tasks",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q -k timeline\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-321, TIME-322."),
            divider(), h2("Next Ticket"), p("TIME-325."),
        ),
    },
    {
        "summary": "TIME-325: AI steps: capture detects steps and parents, Break this down, suggested placement",
        "labels": ["backend", "capture", "llm", "steps"],
        "description": doc(
            h2("Goal"), p("Steps should mostly come from TimeSense, not from the user organizing. Capture recognises an explicit list of actions ('renew passport: get photos, then fill out the form'), recognises 'add get photos to renew passport' as a step of an existing task, and suggests a likely parent for a new task without ever attaching it automatically. Break this down proposes steps that are saved only on approval. When a step joins an ordered group late, TimeSense proposes where it goes."),
            divider(), h2("Scope"), bullet_list([
                "_PARSE_SYSTEM gains steps, steps_in_order and parent_match_id (chosen from up to 40 supplied open task ids/titles, only on an explicit reference); max_tokens raised to ~700; steps and parent match parsed in their own try so bad output never discards the main parse",
                "CaptureRequest gains parent_task_id (the 'Part of' chip), which wins over the LLM match; the server validates any matched id belongs to the user",
                "Capture flow: parent given or matched -> StepService.attach (plus a position suggestion for ordered groups); steps present -> no parent auto-place, create_steps; otherwise suggest_parent fills suggested_parent",
                "POST /tasks/{id}/breakdown: 2-7 suggested steps + sequential, nothing persisted, {steps: [], available: false} on LLM failure",
                "GET /tasks/{id}/step-position?parent_id=: {before_step_id | null}, null on failure",
                "StepService.suggest_parent: returned only above a confidence threshold, never applied",
                "Capture rate limit applies to breakdown and suggestion calls",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No deterministic fallback for breakdown; a made-up breakdown is worse than none",
                "No automatic grouping ever",
                "No Notion changes (TIME-326) and no iOS UI (TIME-328, TIME-329)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/services/capture_service.py, backend/app/api/v1/capture.py, backend/app/schemas/capture*.py",
                "backend/app/services/step_service.py, backend/app/api/v1/tasks.py",
                "backend/tests/test_capture*.py, backend/tests/test_task_breakdown.py (new)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A capture listing several actions toward one outcome creates a parent with those steps, chained when ordered",
                "'add X to Y' attaches X to the user's open task Y; a parent_match_id belonging to another user is ignored",
                "The chip's parent_task_id overrides the LLM's match",
                "Malformed steps JSON still yields the correctly parsed single task",
                "suggested_parent is returned but never applied",
                "Breakdown persists nothing and reports unavailable when the LLM fails",
                "Full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q -k 'capture or breakdown or step'\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-319, TIME-321, TIME-323."),
            divider(), h2("Next Ticket"), p("TIME-326."),
        ),
    },
    {
        "summary": "TIME-326: Notion import keeps sub-items and Blocked by relations",
        "labels": ["backend", "integrations", "notion", "steps", "prerequisites"],
        "description": doc(
            h2("Goal"), p("Notion databases can have sub-items (a 'Parent item' relation) and dependencies ('Blocked by'). Import currently reads only title and due date, so that structure is lost. Keep it: link to tasks already imported, resolve links later when the other side is imported, and offer to import a missing parent alongside its child."),
            divider(), h2("Scope"), bullet_list([
                "notion_source reads the sub-item parent relation and the Blocked by relation when present (by relation type and name)",
                "notion_import_items gains external_parent_id and external_prereq_ids (migration)",
                "import_item resolves links in both directions via created_task_id: attach to an imported parent, add edges to imported prerequisites, and link already-imported children or dependents of this page",
                "Pending list exposes parent_title_hint and parent_pending_item_id for 'Import both'",
                "Items with no Notion parent get suggest_parent, as capture does",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No writing back to Notion",
                "No Apple Reminders work (EventKit is believed not to expose subtasks; TIME-027 remains unbuilt)",
                "No iOS UI (TIME-329)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/app/integrations/notion_source.py, backend/app/integrations/task_source_base.py",
                "backend/app/services/notion_service.py, backend/app/models/notion*.py, backend/app/schemas/notion*.py",
                "backend/migrations/versions/<new>_notion_import_relations.py",
                "backend/tests/test_notion*.py",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A child imported after its parent is attached automatically",
                "A parent imported after its child links the child back",
                "A Blocked by relation between two imported pages becomes a prerequisite edge",
                "A database without these relations imports exactly as before",
                "Migration round-trips on Postgres; full backend suite passes",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest -q -k notion\ncd backend && pytest -q\ndocker compose up -d && cd backend && alembic upgrade head && alembic downgrade -1 && alembic upgrade head"),
            divider(), h2("Dependencies"), p("TIME-321, TIME-322, TIME-325."),
            divider(), h2("Next Ticket"), p("TIME-327."),
        ),
    },
    {
        "summary": "TIME-327: iOS shows step groups, what a step belongs to, and what a task waits for",
        "labels": ["ios", "today", "now", "steps", "prerequisites"],
        "description": doc(
            h2("Goal"), p("If Now just says 'Get photos', the user cannot tell it belongs to Renew passport. Every place a step appears names its parent: the Now card gets an eyebrow 'RENEW PASSPORT · STEP 1 OF 3' above the title. Today shows a group once with its steps inset, and a waiting task dimmed with 'After: <title>'."),
            divider(), h2("Scope"), bullet_list([
                "DTOs: TimelineTask gains parentTaskId, parentTitle, position, stepCount, openStepCount, blockedBy, steps; PrerequisiteRef; NowTask gains parent fields",
                "Now: eyebrow label above the title for a step; tapping it opens the group; swap candidates exclude blocked tasks and parents",
                "Today: StepGroupRow (parent title, '1 of 3' capsule, inset step rows whose checks use the existing Done/duration flow); blocked rows dimmed with an hourglass and 'After: <title>', VoiceOver 'Waiting on <title>'",
                "Completing a parent with open steps asks 'Mark all N steps done?' first",
                "Today context menu entries wired to existing endpoints: Add a step, Remove from group, Don't wait",
                "Today sheets consolidated into one enum-driven .sheet(item:)",
                "Widget snapshot and App Intents show the parent context for a step",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No detail sheet, task picker or Break this down UI (TIME-328)",
                "No Capture changes (TIME-329)",
                "No Android or web changes",
                "No drag and drop",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios/TimeSense/Features/Today/TodayViewModel.swift, TodayView.swift, StepGroupRow.swift (new)",
                "ios/TimeSense/Features/Now/NowViewModel.swift, NowView.swift",
                "ios/TimeSense/Core/Widgets/WidgetSnapshot.swift, ios/TimeSense/Intents/*",
                "ios/TimeSenseTests/*",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "A step on the Now card shows its parent name and step number above the title",
                "Today shows a group once with its steps, and a waiting task dimmed with what it waits for",
                "Completing a step still runs the duration prompt; completing the last step shows the group done",
                "Older-shaped responses (no new fields) still decode",
                "iOS build and tests pass",
            ]),
            divider(), h2("Verification"), code_block("xcodebuild build -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO\nxcodebuild test -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-323, TIME-324."),
            divider(), h2("Next Ticket"), p("TIME-328."),
        ),
    },
    {
        "summary": "TIME-328: iOS task detail sheet, task picker, and Break this down",
        "labels": ["ios", "today", "steps", "prerequisites", "llm"],
        "description": doc(
            h2("Goal"), p("iOS has no place to look at a single task. Add a minimal detail sheet where steps can be added, removed or suggested, where a task can be moved into or out of a group, and where 'Do this after' is set, using one reusable task picker. Keep it light: the product must never become another job to manage."),
            divider(), h2("Scope"), bullet_list([
                "TaskDetailSheet (medium/large detents): steps list with swipe to remove and an inline Add a step field; Break this down showing suggestions as toggles then 'Add N steps'; a Waits for section; a Part of row to change or remove the parent",
                "TaskPickerView (searchable open tasks) reused for Do this after, Make it a step of, and later Capture's Part of chip; a 409 shows inline ('That would make these wait on each other')",
                "When a task joins an ordered group, ask 'Before <step>?' using the step-position suggestion, with 'No particular order'",
                "Today context menu: Break this down, Make it a step of, Do this after, Details",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No editing of title, time or priority in the sheet",
                "No manual step reordering",
                "No Capture changes (TIME-329)",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "ios/TimeSense/Features/Today/TaskDetailSheet.swift (new)",
                "ios/TimeSense/Features/Shared/TaskPickerView.swift (new)",
                "ios/TimeSense/Features/Today/TodayView.swift, TodayViewModel.swift",
                "ios/TimeSenseTests/*",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Break this down shows suggestions and saves only the ones left on",
                "A standalone task can be made a step of another task and removed from it again",
                "Do this after dims the task in Today and removes it from Now until the other task is done",
                "A cycle is explained inline, not as a generic error",
                "iOS build and tests pass",
            ]),
            divider(), h2("Verification"), code_block("xcodebuild test -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-325, TIME-327."),
            divider(), h2("Next Ticket"), p("TIME-329."),
        ),
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Acceptance Criteria"), bullet_list([
                "Capturing 'add get photos to renew passport' shows it added to Renew passport, and Undo makes it standalone",
            ...
            divider(), h2("Verification"), code_block("xcodebuild test -project ios/TimeSense.xcodeproj -scheme TimeSense -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO"),
            divider(), h2("Dependencies"), p("TIME-325, TIME-326, TIME-328."),
    ...
    },
    {
        ...
        "description": doc(
            ...
            divider(), h2("Next Ticket"), p("TIME-331."),
        ),
    },
    {
        "summary": "TIME-331: Fix three backend tests that fail on main (location errand, push cooldown, null push sender)",
        "labels": ["backend", "tests", "bugfix"],
        "description": doc(
            h2("Goal"), p("Three backend tests fail on a clean main checkout, both in the full suite and run alone (seen 2026-09-15 in the evening, local time, during TIME-330): test_location::test_errand_without_maps_never_leads ('Buy groceries at the store' became best_task), and test_push_service::test_pushes_after_cooldown_elapses and test_null_sender_records_nothing_delivered (push_for_user returns None for an overdue priority-1 task). All three read the real clock and none pins 'now', so time of day is the suspected cause. Find the real cause and make the three tests pass at any hour, without weakening what they check. If the cause is a product bug rather than the test, fix the product."),
            divider(), h2("Scope"), bullet_list([
                "Reproduce each failure and confirm the cause (for example quiet hours, a time-of-day score, the user's local day, or deadline-window math). Record it in the PR",
                "If the test depends on the clock: pin 'now' (pass now= where the service accepts it, or patch the clock the engine reads) and keep the assertion unchanged",
                "If the product is wrong: fix it in the service or engine, with a test for the case that exposed it",
                "Check the other clock-sensitive tests already logged (completion-learning pair tests near midnight, test_insights_series weekly workouts) and fix them the same way if the cause is shared; otherwise leave them and log why",
                "Remove the entry from known_issues.md once fixed",
            ]),
            divider(), h2("Non-Goals"), bullet_list([
                "No change to what the tests assert (an unverifiable errand never leads; an eligible push after cooldown is sent; a null sender records a push with 0 delivered)",
                "No change to test_integrations_oauth::test_notion_authorize_503_when_unconfigured (fails only because the local .env configures Notion)",
                "No iOS, Android or web changes",
                "No new recommendation behaviour beyond fixing a confirmed bug",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/tests/test_location.py",
                "backend/tests/test_push_service.py",
                "backend/app/services/push/push_service.py (if the cause is in the product)",
                "backend/app/api/v1/now.py or backend/app/services/recommendation/* (if the cause is in the product)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "The three tests pass on main at any time of day, including when the clock is patched to night, morning and just before midnight",
                "Their assertions are unchanged",
                "The root cause is written in the PR and the implementation log",
                "The full backend suite has no failures other than the .env-dependent Notion test",
                "known_issues.md no longer lists these three tests",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_location.py tests/test_push_service.py -q\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-121 (proactive push), TIME-330 (where the failures were found)."),
            divider(), h2("Next Ticket"), p("TIME-332."),
        ),
    },
    {
        "summary": "TIME-332: Completion-learning tests stop failing at a part-of-day boundary",
        "labels": ["backend", "tests", "bugfix"],
        "description": doc(
            h2("Goal"), p("Three tests in test_completion_learning.py fail when the suite runs within about five minutes after a part-of-day boundary (05:00, 12:00, 18:00 and 21:00 in the user's timezone, which is UTC for the test user): test_completing_a_different_task_records_the_pair, test_the_pair_is_never_pinned and test_a_burst_of_completions_cannot_invent_a_preference. The helper `_show` dates the recommendation five minutes before now, and TaskCompletionService._same_part_of_day only pairs a completion with a recommendation when both fall in the same part of the day, so across a boundary no swap is recorded and the tests fail. Make them pass at any minute of any day, without weakening what they check. Last of the clock-dependent test failures found in TIME-330 and TIME-331."),
            divider(), h2("Scope"), bullet_list([
                "Reproduce by pinning the clock just after a boundary (for example 12:02), and confirm the same-part-of-day rule is the cause. Record it in the PR",
            ...
            divider(), h2("Non-Goals"), bullet_list([
                "No change to the product rule that a completion only teaches within the same part of day (TIME-316)",
                "No change to what the tests assert",
                "No change to test_integrations_oauth::test_notion_authorize_503_when_unconfigured, which fails only because the local .env configures Notion",
                "No iOS, Android or web changes",
            ]),
            divider(), h2("Files Likely Changed"), bullet_list([
                "backend/tests/test_completion_learning.py",
                "backend/app/services/task_completion_service.py (only if the cause turns out to be the product)",
                "docs/project_memory/* and CHANGELOG.md",
            ]),
            divider(), h2("Acceptance Criteria"), bullet_list([
                "The three tests pass with the clock pinned just after 05:00, 12:00, 18:00 and 21:00, and at a mid-part time",
                "Their assertions are unchanged, and the stale-recommendation test still passes",
                "The cause is written in the PR and the implementation log",
                "The full backend suite passes, with no failures other than the .env-dependent Notion test",
                "known_issues.md no longer lists these three tests as unexplained",
            ]),
            divider(), h2("Verification"), code_block("cd backend && pytest tests/test_completion_learning.py -q\ncd backend && pytest -q"),
            divider(), h2("Dependencies"), p("TIME-316 (completion learning), TIME-331 (the same class of clock-dependent failure)."),
            divider(), h2("Next Ticket"), p("(none)"),
        ),
    },
]
